...
class CopyCoordApp(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("座標取得アプリ")
        self.geometry("200x300")  # 縦横のサイズを調整

        # ボタンとエントリーのパディングを調整
        self.start_button = ctk.CTkButton(self, text="開始位置", command=self.capture_start, fg_color="#0078D4", corner_radius=10)
        self.start_button.pack(pady=5, padx=10)
        
        self.end_button = ctk.CTkButton(self, text="終了位置", command=self.capture_end, state="disabled", fg_color="#0078D4", corner_radius=10)
        self.end_button.pack(pady=5, padx=10)
